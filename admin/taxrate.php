...
// Set Document Title
$document->setTitle($language->get('title_taxrate'));
...
<div class="content-wrapper" ng-controller="TaxrateController">
  ...
  <section class="content-header">
    <h1>
      <?php echo $language->get('text_taxrate_title'); ?>
      <small>
        <?php echo store('name'); ?>
      </small>
    </h1>
  ...
  <section class="content">
    ...
    <?php if ($user->getGroupId() == 1 || $user->hasPermission('access', 'create_taxrate')) : ?>
      <div class="box box-info<?php echo create_box_state(); ?>">
        <div class="box-header with-border">
          <h3 class="box-title">
            <span class="fa fa-fw fa-plus"></span> <?php echo $language->get('text_new_taxrate_title'); ?>
          </h3>
      ...
      <div class="col-xs-12">
        <div class="box box-success">
          <div class="box-header">
            <h3 class="box-title">
              <?php echo $language->get('text_taxrate_list_title'); ?>
            </h3>
          </div>
          <div class="box-body">
            <div class="table-responsive">  
              <?php
                  ...
                  if ($user->getGroupId() != 1) {
                    if (! $user->hasPermission('access', 'update_taxrate')) {
                      $hide_colums .= "5,";
                    }
                    if (! $user->hasPermission('access', 'delete_taxrate')) {
                      $hide_colums .= "6,";
                    }
                  }
                ?>  
              <table id="taxrate-taxrate-list" class="table table-bordered table-striped table-hover" data-hide-colums="<?php echo $hide_colums; ?>">
                <thead>
                  <tr class="bg-gray">
                    ...
                    <th class="w-30" >
                      <?php echo $language->get('label_taxrate_name'); ?>
                    </th>
                    <th class="w-20" >
                      <?php echo $language->get('label_taxrate_code'); ?>
                    </th>
                    <th class="w-15">
                      <?php echo $language->get('label_taxrate'); ?>
                    </th>
                     <th class="w-10">
