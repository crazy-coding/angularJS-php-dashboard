...
// Set Document Title
$document->setTitle($language->get('title_currency'));
...
<div class="content-wrapper">
  ...
  <section class="content-header" ng-controller="CurrencyController">
    <h1>
      <?php echo $language->get('text_currency_title'); ?>
      <small>
        <?php echo store('name'); ?>
      </small>
    </h1>
  ...
  <section class="content">
    ...
    <?php if ($user->getGroupId() == 1 || $user->hasPermission('access', 'create_currency')) : ?>
      <div class="box box-info<?php echo create_box_state(); ?>">
        <div class="box-header with-border">
          <h3 class="box-title">
            <span class="fa fa-fw fa-plus"></span> <?php echo $language->get('text_new_currency_title'); ?>
          </h3>
      ...
      <div class="col-xs-12">
        <div class="box box-success">
          <div class="box-header">
            <h3 class="box-title">
              <?php echo $language->get('text_currency_list_title'); ?>
            </h3>
          </div>
          <div class="box-body">
            <div class="table-responsive">  
              <?php
                  ...
                  if ($user->getGroupId() != 1) {
                    if (! $user->hasPermission('access', 'update_currency')) {
                      $hide_colums .= "7,";
                    }
                    if (! $user->hasPermission('access', 'delete_currency')) {
                      $hide_colums .= "8,";
                    }
                    if (! $user->hasPermission('access', 'change_currency')) {
                      $hide_colums .= "9,";
                    }
                  }
                ?> 
              <table id="currency-currency-list" class="table table-bordered table-striped table-hover" data-hide-colums="<?php echo $hide_colums; ?>">
                <thead>
                  <tr class="bg-gray">
                    ...
                    <th class="w-5">
                      <?php echo $language->get('label_decimal_place'); ?>
                    </th>
                    <th class="w-10">
