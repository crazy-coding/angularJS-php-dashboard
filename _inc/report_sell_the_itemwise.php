...
// DB table to use
$table = "(SELECT selling_info.created_at, selling_item.id, selling_item.item_id, selling_item.item_name, selling_item.item_quantity as total_item, selling_item.item_discount as discount, selling_item.item_tax as tax, selling_item.total_buying_price as buy_price, selling_item.item_total as sell_price FROM selling_item 
...
$columns = array(
    ...
    array( 
      ...
      'dt' => 'item_name',
      'formatter' => function( $d, $row ) {
        return '<a href="product.php?p_id=' . $row['item_id'] . '&p_name=' . ucfirst($row['item_name']) . '">' . ucfirst($row['item_name']) . '</a>';
      }
    ),
    ...
    array( 
      ...
      'dt' => 'sell_price',
      'formatter' => function( $d, $row ) {
        $total = $row['sell_price'] - $row['discount'];
        return currency_format($total);
    ...
    array( 
      ...
      'dt' => 'discount',
      'formatter' => function( $d, $row ) {
        return currency_format($row['discount']);
    ...
    array( 
      ...
      'dt' => 'profit',
      'formatter' => function( $d, $row ) {
        $total = ($row['sell_price'] - $row['buy_price']) - $row['discount'];
        return currency_format($total);
      }
    )
);
