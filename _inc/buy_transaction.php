...
{
    $id = $request->get['id'];
    $statement = $db->prepare("SELECT * FROM `supplier_transactions` WHERE `id` = ?");
    $statement->execute(array($id));
    $transaction = $statement->fetch(PDO::FETCH_ASSOC);
    include 'template/buy_transaction_view.php';
    exit();
}
...
if (isset($request->get['sup_id']) && $request->get['sup_id'] != 'null') {
  $where_query .= " AND supplier_transactions.sup_id=".$request->get['sup_id'];
}

if (from()) {
$from = from();
$to = to();
$where_query .= date_range_buy_transaction_filter($from, $to);
}

// DB table to use
$table = "(SELECT * FROM supplier_transactions 
